  ...
  return (
    <>
      ...
      {device === 'desktop' ? (
        ...
        <Dropdown
          checkIsSelected={({ value }) => fieldsStore.workFormat.includes(value)}
          isClearable
          mode="light"
          onSelect={onFilterChanged(value => fieldsStore.setWorkFormat(value?.value))}
          options={variants}
          placeholder="Выберите формат работы"
          selectedValue={variants.filter(({ value }) => fieldsStore.workFormat.includes(value)).map(({ name }) => name).join(', ')}
        />
      )}
    </>
  );
