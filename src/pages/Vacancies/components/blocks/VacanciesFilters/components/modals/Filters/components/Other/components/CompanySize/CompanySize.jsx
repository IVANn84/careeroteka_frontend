  ...
  return (
    <>
      ...
      {device === 'desktop' ? (
        ...
        <Dropdown
          checkIsSelected={({ value }) => fieldsStore.companySize.includes(value)}
          isClearable
          mode="light"
          onSelect={onFilterChanged(value => fieldsStore.setCompanySize(value?.value))}
          options={variants}
          placeholder="Выберите размер компании"
          selectedValue={variants.filter(({ value }) => fieldsStore.companySize.includes(value)).map(({ name }) => name).join(', ')}
        />
      )}
    </>
  );
