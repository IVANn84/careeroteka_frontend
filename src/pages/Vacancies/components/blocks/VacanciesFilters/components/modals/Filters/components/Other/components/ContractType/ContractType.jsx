  ...
  return (
    <>
      ...
      {device === 'desktop' ? (
        ...
        <Dropdown
          checkIsSelected={({ value }) => fieldsStore.contractType.includes(value)}
          isClearable
          mode="light"
          onSelect={onFilterChanged(value => fieldsStore.setContractType(value?.value))}
          options={variants}
          placeholder="Выберите формат трудоустройства"
          selectedValue={variants.filter(({ value }) => fieldsStore.contractType.includes(value)).map(({ name }) => name).join(', ')}
        />
      )}
    </>
  );
