...
const prepareArguments = (args = {}, methodName) => args.map((number,
    index) => checkCorrect(number, index, methodName));
...
const plus = (...args) => {
    if (args?.length) {
        ...
        const bigNumbers = prepareArguments(args, methodName);
        const calculating = () => bigNumbers.reduce((acc = Big(0), currentNumber) => currentNumber.plus(acc));
...
const minus = (minuend, subtrahend) => {
    ...
    const bigNumbers = prepareArguments([minuend, subtrahend], methodName);
...
const divide = (dividend, divider) => {
    ...
    const bigNumbers = prepareArguments([dividend, divider], methodName);
...
const times = (multiplied, multiplier) => {
    ...
    const bigNumbers = prepareArguments([multiplied, multiplier], methodName);
...
const toFixed = (number, precision = 2, roundMod = 1) => {
    ...
    */
    ...
    const bigNumbers = prepareArguments([number], methodName);
...
const lt = (compared, comparer) => {
    const methodName = 'lt';
    const bigNumbers = prepareArguments([compared, comparer], methodName);
